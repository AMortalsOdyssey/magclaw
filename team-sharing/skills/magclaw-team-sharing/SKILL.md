...
0. If the user says the current session should not report to MagClaw, or uses equivalent wording such as "这个 session 不上报", treat it as a Team Sharing session-reporting control. The hooks also detect this before upload, but if the current transcript path/session id is available you may immediately run `team-sharing session-reporting off --transcript <path> --session-id <id>` from the project. If the user later says this session can start reporting again, run `team-sharing session-reporting on ...`; the next hook upload starts from that enable message and does not backfill earlier disabled content.

...
3. Add retrieval filters when the user gives a time or search preference. Keep the default retrieval combined and server-wide: keyword/BM25 and semantic/vector recall run together; current Channel and same-Server other Channels are recalled, deduplicated, then reranked.
   - Time: `--time today`, `--time yesterday`, `--time this-week`, or explicit `--from <iso> --to <iso>`.
   - Scope: omit `--scope` for the default `hybrid` search across the current Channel plus other Channels in the same Server. Use `--scope channel` only for old current-Channel-only behavior, or `--scope server` when you intentionally want one whole-Server recall path.
...
- Use `retrievalScope` and `sameChannel` when present to label whether an entry came from the current Channel or another Channel in the same Server; do not hide useful cross-Channel evidence just because it is outside the current Channel.

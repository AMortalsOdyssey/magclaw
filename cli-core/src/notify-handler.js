  ...
  const presentation = {
    ...
    blocks: [
      ...
      { type: 'buttons', buttons: [
        { label: '确认', action: { type: 'command', command: `magclaw-notify-handler confirm --profile ${safePart(state.profile)} --id ${confirmation.id} --approve` } },
        { label: '拒绝', action: { type: 'command', command: `magclaw-notify-handler confirm --profile ${safePart(state.profile)} --id ${confirmation.id} --reject` } },
      ] },
    ],
  };
...
export async function handleNotifyDelivery(profilePaths, request) {
  ...
  if (!state.config.enabled) {
    return { requestId: request.id, status: 'awaiting_configuration', publicReason: 'Notify handler is disabled.', localReceiptId: receiptId };
  }
  ...
  if (groupResolution.status === 'unavailable') {
    ...
    const result = { requestId: request.id, status, publicReason: status === 'target_unavailable' ? 'The requested target is unavailable.' : 'Notify groups are not configured.', localReceiptId: receiptId };
  ...
  if (groupResolution.status === 'confirmation_required') {
    ...
    await sendConfirmationPrompt(state, confirmation).catch(() => {});
    const result = { requestId: request.id, status: 'awaiting_confirmation', publicReason: 'The requested target requires owner confirmation.', localReceiptId: receiptId };
    await appendReceipt(state, { ...result, confirmationId: confirmation.id, createdAt: now() });
    return result;
  }
  const group = groupResolution.group;
  if (!group.chatId) {
    const result = { requestId: request.id, status: 'awaiting_configuration', publicReason: 'The resolved group is not fully configured.', localReceiptId: receiptId };
    await appendReceipt(state, { ...result, createdAt: now() });
    return result;
  }
...
async function reportNotifyResultToCloud(state, result) {
  ...
  const serverUrl = cleanText(daemonConfig.serverUrl, 1000).replace(/\/+$/, '');
  const token = cleanText(daemonConfig.token || daemonConfig.machineToken || daemonConfig.apiKey, 2000);
  if (!serverUrl || !token) return { reported: false, reason: 'daemon_cloud_auth_unavailable' };
  try {
    const response = await fetch(`${serverUrl}/api/notify/internal/result`, {
      method: 'POST',
      headers: {
...
export async function registerNotifyRoute(profilePaths) {
  const state = await ensureNotifyHandlerState(profilePaths);
  const daemonConfig = jsonObject(await readJson(state.profilePaths.config, {}));
  const serverUrl = cleanText(daemonConfig.serverUrl, 1000).replace(/\/+$/, '');
  const token = cleanText(daemonConfig.token || daemonConfig.machineToken || daemonConfig.apiKey, 2000);
  if (!serverUrl || !token) throw new Error('Daemon cloud authentication is not configured for this profile.');
  const response = await fetch(`${serverUrl}/api/notify/internal/route`, {
    method: 'POST',
  ...
  if (!response.ok) throw new Error(body.error || `Notify route registration returned HTTP ${response.status}.`);
  return body.route;
}
...
export async function confirmNotifyMapping(profilePaths, confirmationId, decision, options = {}) {
  ...
  if (!['approve', 'reject'].includes(decision)) throw new Error('Notify confirmation decision must be approve or reject.');
  ...
  if (!Number.isFinite(Date.parse(record.createdAt)) || Date.now() - Date.parse(record.createdAt) > CONFIRMATION_TTL_MS) {
  ...
  const storedRequest = await readJson(path.join(state.paths.requestDir, `${safePart(record.requestId)}.json`), null);
  if (!storedRequest) throw new Error('The original Notify request is unavailable.');
  const result = await handleNotifyDelivery(profilePaths, storedRequest);
  const cloudReport = await reportNotifyResultToCloud(state, result);
  record.result = result;
  record.cloudReport = cloudReport;
  await writeJson(state.paths.pending, pending);

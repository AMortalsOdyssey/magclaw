...
export function buildCodexLocalDigestPrompt({ session = {}, events = [] } = {}) {
  ...
  return [
    ...
    '',
    '# Output',
    'Return concise Markdown with these headings: `Task / Topic`, `Important Context`, `What Was Done`, `Validation / Evidence`, `Current Conclusions`, `Important Discoveries`, `Open Questions`, `Useful Artifacts`, `Source Event IDs`.',
    '',
    '# Rules',
    '- Summarize what actually happened in the conversation, with priority on the final assistant reply when it exists.',
    '- `What Was Done`, `Validation / Evidence`, `Current Conclusions`, and `Important Discoveries` should each be one short sentence when that signal exists.',
    '- `Validation / Evidence` must explain what the checks imply, not just list command names.',
    '- `Important Discoveries` is for surprising findings, constraints, risks, or unrelated but material issues found during the work.',
...
export function buildTeamSharingSummaryPrompt({ session = {}, events = [], previousAbstract = '' } = {}) {
  ...
  return [
    ...
    '- If a user prompt contains browser comments, screenshot notes, or selected page evidence, treat every comment as an explicit acceptance item and preserve the concrete UI/behavior request.',
    '- If the new events correct or refine earlier conclusions, update the abstract instead of appending contradictory text.',
    '- For every substantial final assistant reply, extract these four reusable dimensions when present: what was changed or investigated, what validation showed, what conclusion now holds, and what important discovery or risk appeared.',
    '- 每个维度用一句话即可；不要只复述用户诉求，也不要把工具清单当成总结。',
    ...
    '- 如果事件里包含“页面批注”“选取片段”“附件与截图”，必须把这些批注映射到具体问题和修复要求；不要只总结 My request 的最后一段。',

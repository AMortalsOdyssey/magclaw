...
function defaultCliPackageSpec(env = process.env) {
  return String(env.MAGCLAW_CLI_PACKAGE_SPEC || '@magclaw/daemon@latest').trim() || '@magclaw/daemon@latest';
}
...
export function renderCliShimFiles({ platform = process.platform, npmPath = '', packageSpec = '@magclaw/daemon@latest' } = {}) {
  const targetPackage = String(packageSpec || '@magclaw/daemon@latest').trim() || '@magclaw/daemon@latest';
  ...
  return [
    {
      ...
      content: [
        '#!/bin/sh',
        'set -eu',
        '# MagClaw CLI shim generated by @magclaw/daemon.',
...
async function existingDurableMagclawCommand(env = process.env) {
  ...
  if (content && !content.includes('MagClaw CLI shim generated by @magclaw/daemon') && !content.includes('@magclaw/daemon@')) {
    return '';
  }
  return existing;
}
...
async function writeCliShimFile(file, content, { force = false } = {}) {
  if (existsSync(file) && !force) {
    ...
    if (!existing.includes('MagClaw CLI shim generated by @magclaw/daemon') && !existing.includes('@magclaw/daemon@')) {
...
async function installCliShim(options = {}, env = process.env) {
  ...
  const packageSpec = String(options.packageSpec || options.cliPackageSpec || defaultCliPackageSpec(env)).trim() || '@magclaw/daemon@latest';

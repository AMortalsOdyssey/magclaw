...
test('sidebar settings and skill panels support collapsible MagClaw UI sections', async () => {
  ...
  assert.match(app, /release: \{ title: 'Release Notes', icon: 'release', section: '' \}/);
  assert.match(app, /meta\.section \? `<div class="settings-section-label">/);
  ...
  assert.doesNotMatch(app, /Versioned changelog/);
...
test('agent activity tab renders newest first with second-level timestamps and a 5000 item cap', async () => {
  ...
  const activityTabSource = app.slice(app.indexOf('function renderAgentActivityTab'), app.indexOf('function agentDetailTabIsLoading'));

  assert.match(app, /const AGENT_ACTIVITY_EVENT_LIMIT = 5000/);
  assert.match(app, /const AGENT_ACTIVITY_HISTORY_DAYS = 7/);
  assert.match(app, /let agentActivityCache = \{\}/);
  assert.match(app, /function agentActivityEvents\(agent\)/);
  assert.match(app, /function agentActivityCacheFor\(agentId\)/);
  assert.match(app, /function loadAgentActivity\(agentId, \{ force = false \} = \{\}\)/);
  assert.match(app, /\/api\/agents\/\$\{encodeURIComponent\(agentId\)\}\/activity/);
  assert.match(app, /params\.set\('days', String\(AGENT_ACTIVITY_HISTORY_DAYS\)\)/);
  assert.match(app, /params\.set\('limit', String\(AGENT_ACTIVITY_EVENT_LIMIT\)\)/);
  assert.match(app, /if \(normalized === 'activity'\)[\s\S]*await loadAgentActivity\(agentId\)/);
  ...
  assert.match(app, /agentRuntimeErrorMessage\(agent\)/);
  assert.match(liveActivitySource, /latestEvent\?\.raw\?\.activity\?\.detail/);
  assert.match(liveActivitySource, /Error: \$\{errorMessage\}/);
  ...
  assert.match(app, /data-action="copy-agent-activity-diagnostic"/);
  assert.match(activityTabSource, /Copy Diagnostic Info/);
  assert.match(profileSource, /renderAgentLiveActivityBar\(agent\)/);
  assert.match(profileSource, /renderAgentErrorNotice\(agent\)/);

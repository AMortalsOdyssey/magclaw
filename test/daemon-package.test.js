...
test('daemon version and foreground log lines are structured', async () => {
  const cliCorePackage = JSON.parse(await readFile(new URL('../cli-core/package.json', import.meta.url), 'utf8'));
  assert.equal(DAEMON_VERSION, cliCorePackage.version);
  assert.equal(
...
test('daemon status reports package update notices without running the upgrade worker', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'magclaw-daemon-package-update-status-'));
  try {
    const env = {
      ...process.env,
      MAGCLAW_DAEMON_HOME: home,
      MAGCLAW_ENTRY_PACKAGE_NAME: '@magclaw/daemon',
      MAGCLAW_ENTRY_PACKAGE_VERSION: '0.1.41',
      MAGCLAW_PACKAGE_UPDATE_TTL_MS: '60000',
      MAGCLAW_PACKAGE_UPDATE_DISABLE_NETWORK: '1',
      MAGCLAW_PACKAGE_UPDATE_LATEST__MAGCLAW_DAEMON: '9.9.9',
      MAGCLAW_PACKAGE_UPDATE_LATEST__MAGCLAW_CLI_CORE: '9.9.9',
    };
    const result = spawnSync(process.execPath, [
      DAEMON_BIN,
      'status',
    ...
    assert.equal(payload.packageUpdate.latestVersion, '9.9.9');
    assert.equal(payload.packageUpdate.updateAvailable, true);
    assert.equal(payload.packageUpdate.action, 'notice');
    assert.equal(payload.packageUpdate.applyCommand, 'magclaw upgrade --profile update-status --to 9.9.9');
...
test('computer status reports computer package update commands without auto-upgrading', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'magclaw-computer-package-update-status-'));
  try {
    const env = {
      ...process.env,
      MAGCLAW_DAEMON_HOME: home,
      MAGCLAW_ENTRY_PACKAGE_NAME: '@magclaw/computer',
      MAGCLAW_ENTRY_PACKAGE_VERSION: '0.1.40',
      MAGCLAW_DAEMON_PACKAGE_NAME: '@magclaw/computer',
      MAGCLAW_DAEMON_PACKAGE_KIND: 'computer',
      MAGCLAW_DAEMON_PACKAGE_BIN: 'magclaw-computer',
      MAGCLAW_PACKAGE_UPDATE_DISABLE_NETWORK: '1',
      MAGCLAW_PACKAGE_UPDATE_LATEST__MAGCLAW_COMPUTER: '9.9.9',
      MAGCLAW_PACKAGE_UPDATE_LATEST__MAGCLAW_CLI_CORE: '9.9.9',
    };
    const result = spawnSync(process.execPath, [
      COMPUTER_BIN,
    ...
    assert.equal(payload.packageUpdate.latestVersion, '9.9.9');
    assert.equal(payload.packageUpdate.action, 'notice');
    assert.equal(payload.packageUpdate.applyCommand, 'magclaw-computer upgrade --profile computer-update --target-version 9.9.9');
    assert.equal(payload.cliCoreUpdate.packageName, '@magclaw/cli-core');
    assert.equal(payload.cliCoreUpdate.action, 'notice');
    assert.equal(payload.service?.pendingCommandId, undefined);
  } finally {
...
test('daemon renders durable MagClaw CLI shims for macOS Linux and Windows', () => {
  ...
  assert.deepEqual(macFiles.map((file) => file.name), ['magclaw', 'magclaw-computer', 'magclaw-notify-handler']);
  ...
  assert.match(macFiles[2].content, /exec "\$NPM_BIN" exec --yes --package "\$PACKAGE_SPEC" -- magclaw-notify-handler "\$@"/);
  ...
  assert.deepEqual(linuxFiles.map((file) => file.name), ['magclaw', 'magclaw-computer', 'magclaw-notify-handler']);
  ...
  assert.deepEqual(windowsFiles.map((file) => file.name), [
    ...
    'magclaw-notify-handler.cmd',
    'magclaw-notify-handler.ps1',
  ]);
...
test('install-cli command writes durable MagClaw command shims', async () => {
  ...
  try {
    ...
    assert.deepEqual(payload.commands, ['magclaw', 'magclaw-computer', 'magclaw-notify-handler']);
    ...
    if (process.platform === 'win32') {
      ...
      assert.ok(files.includes('magclaw-notify-handler.cmd'));
      assert.ok(files.includes('magclaw-notify-handler.ps1'));
    } else {
      assert.deepEqual(files, ['magclaw', 'magclaw-computer', 'magclaw-notify-handler']);
...
test('install-cli repairs missing companion shims next to existing magclaw', async () => {
  ...
  try {
    ...
    const files = await readdir(binDir);
    assert.deepEqual(files.sort(), ['magclaw', 'magclaw-computer', 'magclaw-notify-handler']);
    const computerShim = await readFile(path.join(binDir, 'magclaw-computer'), 'utf8');
    assert.match(computerShim, /@magclaw\/computer@latest/);
  } finally {
    await rm(home, { recursive: true, force: true });

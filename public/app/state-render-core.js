...
const CONVERSATION_HISTORY_TOP_THRESHOLD = 96;
...
const PACKAGE_VERSION_CACHE_KEY = 'magclawPackageVersions:v1';
const PACKAGE_VERSION_CACHE_TTL_MS = 10 * 60_000;
...
let knowledgeSpaceState = { loading: false, error: '', data: null, selectedDocId: knowledgeRoute.docId || '', tab: knowledgeRoute.view || 'home', qaQuery: '', qaResult: null, alignText: '', alignResult: null, graphSearchQuery: '' };
let knowledgeSettingsState = { addOpen: false, selectedAddIds: [], removeHumanId: '' };
let knowledgeAgentLinkState = { docId: '', copied: false };
let knowledgePublishState = { sessionId: '' };
let modal = null;
...
let searchChannelPathResolveState = { key: '', raw: '', loading: false, error: '', result: null };
let searchChannelPathRequestSeq = 0;
let searchChannelPathTimer = null;
...
let conversationHistoryPages = { main: {}, thread: {} };
let conversationHistoryLoading = { main: {}, thread: {} };
let taskHistoryPages = { space: {}, global: null };
let taskHistoryLoading = { space: {}, global: false };
...
let eventSourcePath = '';
let eventSourceSuspendedForHiddenTab = false;
let eventSourceResumeInFlight = false;
const SSE_LAST_SEQ_STORAGE_KEY = 'magclaw:last-sse-seq';
let lastSseSeq = Number(sessionStorage.getItem(SSE_LAST_SEQ_STORAGE_KEY) || 0);
...
const AGENT_ACTIVITY_HISTORY_DAYS = 7;
...
function routeStateFromLocation(path = window.location.pathname || '') {
  ...
  const knowledgeDocMatch = value.match(/^\/s\/[^/]+\/knowledge\/docs\/([^/]+)/);
  if (knowledgeDocMatch) {
    return { activeView: 'knowledge', railTab: 'knowledge', knowledgeRoute: { view: 'docs', docId: decodeURIComponent(knowledgeDocMatch[1] || ''), changeSessionId: '' } };
  }
  const knowledgeReviewMatch = value.match(/^\/s\/[^/]+\/knowledge\/reviews\/([^/]+)/);

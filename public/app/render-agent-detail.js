...
function renderAgentSkillSections(skills, { compact = false } = {}) {
  return `
    <div class="agent-skill-section-stack ${compact ? 'compact' : ''}">
      ${renderSkillList('Agent Workspace Skills', skills?.workspace || [], 'No agent workspace skills installed yet.', 'agent-skills')}
...
function renderTeamSharingWorkspaceMarkdown(file = {}) {
  ...
  return html
    .replace(/<a href="#team-sharing-workspace-file:([^"]+)"(?: target="_blank" rel="noreferrer")?>([\s\S]*?)<\/a>/g, (_match, encodedPath, label) => {
      ...
      const target = decodeURIComponent(encodedTarget).split('#')[0];
